...
const getWorkspaceAndItsProjects = async (req, res, next) => {
  try {
    ...
    const workspace = await workspaceService.getWorkspaceAndItsProjects(
      workspaceMember.toJSON(),
    );
    if (!workspace.id)
      throw boom.badImplementation(
        'get workspace and its projects operation returns null',
      );
...
const getWorkspacesAndProjects = async (req, res, next) => {
  try {
    ...
    const workspacesAndProjects =
      await workspaceService.getWorkspacesAndProjects(userId);
...
const createWorkspace = async (req, res, next) => {
  try {
    ...
    if (!workspace)
      throw boom.badRequest('create workspace operation returns null');
...
const deleteWorkspace = async (req, res, next) => {
  try {
    ...
    if (deletedWorkspace === 0)
      throw boom.internal('Delete workspace operation returns 0');
