const TeamMemberDto = require('./teamMember.dto');
...
class TeamDto {
  ...
  static fromModel(team, projectMembers) {
    return {
      ...
      members: team.teamMembers.map((teamMember) => ({
        ...
        isMemberProject: projectMembers.some(
          (pm) => pm.workspaceMemberId === teamMember.workspaceMemberId,
        ),
      })),
    };
  }
  ...
  static WithData(team, requesterAsWorkspaceMember) {
    ...
    return {
      ...
      requesterActions: this.getRequesterActions(
        team.teamMembers,
  ...
  static getOwner(members) {
    ...
    return owner
      ? { name: owner.workspaceMember.user.name }
      : { name: 'unknown' };
  }

  static getRequesterActions(members, requesterAsWorkspaceMember) {
    const requester = members.find(
      (member) => member.workspaceMemberId === requesterAsWorkspaceMember.id,
    );
    if (!requester) return {};
