...
class CardDto {
  constructor({
    ...
    if (Array.isArray(members)) {
      this.cardMembers =
        members.length > 0 ? CardDto.formatMembers(members) : [];
    }
    if (Array.isArray(labels)) {
      this.labels =
        labels.length > 0 ? labels.map((label) => new LabelDto(label)) : [];
    }
    if (Array.isArray(attachments)) {
      this.attachmentsCount = Array.isArray(attachments)
        ? attachments.length
        : 0;
    }
  ...
  static withChecklists(card) {
    return {
      cardId: card.id,
      name: card.name,
      checklists: card.checklists.map(
        (checklist) => new ChecklistDto(checklist),
      ),
    };
  }
  ...
  static withAllCardInformation(card) {
    return {
      ...
      labels:
        card.list?.project?.labels?.map((label) => {
          const labelFormatted = label.get({ plain: true });
          const crd = label.cards.find((c) => c.CardLabel.cardId === card.id);
          return new LabelDto({
            ...labelFormatted,
            isVisible: crd ? crd.CardLabel.isVisible : false,
          });
        }) || [],
      cardMembers: this.formatMembers(card.members),
      attachments:
        card.attachments?.map(
          (attachment) => new CardAttachmentDto(attachment),
        ) || [],
      checklists:
        card.checklists?.map((checklist) => new ChecklistDto(checklist)) || [],
    };
  }
