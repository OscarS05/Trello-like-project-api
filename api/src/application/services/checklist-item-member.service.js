...
class ChecklistItemMemberService {
  constructor({ getAllItemMembersUseCase, getItemMembersByIdsUseCase, addMemberToItemUseCase, deleteChecklistItemUseCase }, { checkListItemByIdAndProjectUseCase }){
  ...
  async getItemMembersByIds(checklistItemId, checklistItemMemberIds){
    return await this.getItemMembersByIdsUseCase.execute(checklistItemId, checklistItemMemberIds);
  }
  ...
  async addMemberToChecklistItem(requesterAsProjectMember, checklistItemId, availableMembersToBeAddedIds){
    ...
    const addedMembers = await this.addMemberToItemUseCase.execute(checklistItemId, availableMembersToBeAddedIds);
    if(addedMembers.length === 0) throw boom.badData('No members were added to the checklist item');

    const addedMembersIds = addedMembers.map((member) => member.id);
    const newMembers = await this.getItemMembersByIds(checklistItemId, addedMembersIds);
    if(newMembers.length === 0) throw boom.badData('No members were added to the checklist item');
