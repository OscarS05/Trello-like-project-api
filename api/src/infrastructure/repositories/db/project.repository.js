...
class ProjectRepository extends IProjectRepository {
  ...
  async create(projectEntity, projectMemberEntity) {
    ...
    try {
      const project = await this.db.models.Project.create(projectEntity, {
        transaction,
      });
      await this.db.models.ProjectMember.create(projectMemberEntity, {
        transaction,
      });
  ...
  async update(projectId, projectUpdateEntity) {
    return this.db.models.Project.update(projectUpdateEntity, {
      where: { id: projectId },
      returning: true,
    });
  }
  ...
  async delete(projectId) {
    return this.db.models.Project.destroy({ where: { id: projectId } });
  }

  async findById(projectId) {
    return this.db.models.Project.findOne({ where: { id: projectId } });
  }
  ...
  async countProjects(workspaceMemberId) {
    return this.db.models.ProjectMember.count({
      where: { workspaceMemberId },
    });
  }
}
