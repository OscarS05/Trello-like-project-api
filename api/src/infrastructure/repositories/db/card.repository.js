...
class CardRepository extends ICardRepository {
  ...
  async create(listId, cardEntity) {
    return this.db.models.Card.create(cardEntity, { where: { listId } });
  }

  async update(cardId, cardUpdateEntity) {
    return this.db.models.Card.update(cardUpdateEntity, {
      where: { id: cardId },
      returning: true,
    });
  }
  ...
  async delete(cardId) {
    return this.db.models.Card.destroy({ where: { id: cardId } });
  }

  async findOneById(cardId) {
    return this.db.models.Card.findOne({ where: { id: cardId } });
  }

  async findOneByIdWithList(cardId) {
    return this.db.models.Card.findOne({
      where: { id: cardId },
      include: [{ model: this.db.models.List, as: 'list' }],
    });
  }

  async findAll(listId) {
    return this.db.models.Card.findAll({ where: { listId } });
  }
